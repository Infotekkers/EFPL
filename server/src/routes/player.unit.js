...
const [reqaddPlayer, reqaddScore, requpdatePlayer, requpdateScore, gameweekId, playerId] = require("../utils/players.test.data")
...
describe("Testing Players",()=>{
        ...
        test("PATCH /players/updateplayer/:playerId SUCCESS", async()=>{
            //send request
            const res = await req.patch(`/players/updateplayer/${playerId}`).send(requpdatePlayer);
            // expect response
            expect(res.statusCode).toBe(201);
        ...
        // send request
            const res = await req.patch(`/players/addscore/${playerId}`).send(reqaddScore);
            // expect response
            expect(res.statusCode).toBe(201);
